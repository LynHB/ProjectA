...
public class GetTableNames {
    public static Pattern p = Pattern.compile("(?ms)('(?:''|[^'])*')|--.*?$|/\\*.*?\\*/|#.*?$|");
    ...
    // 要 校验的 sql
    public static String sql = "select * from user u left join bbb b where u.id=b.id and   u.aaa in (select * from ccc);";
